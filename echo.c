...
on_read (uv_stream_t *client, ssize_t nread, const uv_buf_t *buf)
{
  ...
  req->data = buf->base;
  ((uv_buf_t *)(buf->base + buf->len))->len = nread;
  uv_write (req, client, (void *)(buf->base + buf->len), 1, on_write);
}

static void
on_ralloc (uv_handle_t *handle, size_t siz, uv_buf_t *buf)
{
  buf->base = malloc$ (siz, sizeof (uv_buf_t));
  buf->len = siz;
  ((uv_buf_t *)(buf->base + siz))->base = buf->base;
  ((uv_buf_t *)(buf->base + siz))->len = buf->len;
}
